...
if 'gpt' in model_name and 'azure' not in model_name:
    output_folder = f"/NS/ai-agents/nobackup/afkhan/LLM_Preference_Analysis/Outputs/Type_Case_Study_1_AllSides/azure--{model_name.split('/')[-1]}/Left_Right_Swap/{seed}/"
    os.makedirs(output_folder, exist_ok=True)
else:
    output_folder = f"/NS/ai-agents/nobackup/afkhan/LLM_Preference_Analysis/Outputs/Type_Case_Study_1_AllSides/{model_name.split('/')[-1]}/Left_Right_Swap/{seed}/"
    os.makedirs(output_folder, exist_ok=True)
...
def make_prompt(PROMPT, topic, collated_data, idx):
    ...
    modified_leanings[left_idx] = 'right'
    modified_leanings[right_idx] = 'left'
